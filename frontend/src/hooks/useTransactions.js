...
export const useTransactions = () => {
  ...
  const fetchTransactions = async (options = {}) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3004/api'}/transactions?${params}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.data?.transactions)) {
          ...
          console.log(`✅ ${apiTransactions.length} transactions récupérées depuis l'API`);
      ...
      console.error('❌ Erreur API transactions:', apiError.message);
      ...
      console.log('🏠 Utilisation des transactions locales en fallback');
      return localData;
    } finally {
      setIsLoading(false);
    }
  ...
  const addTransaction = useCallback(async (amount, category, description) => {
    try {
      ...
      if (!token) {
        ...
        console.log('🏠 Transaction ajoutée localement (mode offline)');
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3004/api'}/transactions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        ...
        if (data.success && data.data?.transaction) {
          ...
          console.log(`✅ Transaction créée via API: ${amount}€ (${category})`);
          console.log(`🎉 Points gagnés: ${data.data.pointsEarned}, Streak: ${data.data.newStreak}`);
      ...
      console.error('❌ Erreur création transaction:', apiError.message);
      setError(apiError.message);
      ...
      console.log('🏠 Transaction créée localement en fallback');
  ...
  const removeTransaction = useCallback(async (id) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3004/api'}/transactions/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const getTransactionStats = useCallback(async (period = 'month') => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3004/api'}/transactions/stats?period=${period}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
