...
const AlimentationScreen = ({ userData, setUserData, usePlan, showNotification, addTransaction, darkMode }) => {
  ...
  const generateShoppingList = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3004'}/api/shopping-list/generate`, {
        method: 'POST',
        headers: {
              ...
              <div className="space-y-4">
                <div>
                  <input
                    ...
                    className={`w-full px-4 py-3 rounded-xl border transition-all focus:outline-none focus:ring-2 ${
                      darkMode
                        ? 'bg-gray-900/50 text-white border-gray-700 focus:border-green-500 focus:ring-green-500/20'
                        : 'bg-white text-black border-gray-300 focus:border-green-500 focus:ring-green-500/20'
                    }`}
                  />
                </div>
                ...
                <div className={`p-4 rounded-xl ${
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-lg font-bold text-green-500">{maxPrice}€</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                  ...
                  <button
                    ...
                    className={`w-full py-3 px-6 rounded-xl font-medium transition-all duration-200 ${
                      favorites.length === 0 || isGeneratingList
                        ...
                        : 'bg-gradient-to-r from-green-500 to-yellow-500 text-white hover:scale-105'
                    }`}
                  >
                    {isGeneratingList
                      ? '🤖 Génération en cours...'
    ...
    <div className={`min-h-screen relative ${
          ...
          <div className="flex items-center justify-between">
            ...
            {isLoadingAI && (
              <div className="flex items-center space-x-2">
                <div className="w-4 h-4 rounded-full bg-gradient-to-r from-green-500 to-yellow-500 animate-pulse"></div>
                  ...
                  <div className="flex space-x-2 overflow-x-auto pb-1 scrollbar-hide">
                    {categories.map((category, index) => (
                      <button
                        ...
                        className={`group relative flex items-center space-x-2 px-4 py-3 rounded-xl whitespace-nowrap text-sm font-semibold transition-all duration-300 min-w-fit transform hover:scale-105 ${
                          activeCategory === category.id
                            ? 'bg-gradient-to-r from-green-600 to-yellow-600 text-white shadow-lg shadow-green-500/25 scale-105'
                            : darkMode
                      ...
                      >
                        ...
                        {/* Hover gradient */}
                        <div className="absolute inset-0 bg-gradient-to-r from-green-500/5 to-yellow-500/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                      </button>
                    ))}
                  </div>
                </div>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute top-20 right-10 w-32 h-32 rounded-full ${
          darkMode ? 'bg-green-600/5' : 'bg-green-600/3'
        } blur-3xl animate-pulse`}></div>
        <div className={`absolute bottom-20 left-10 w-40 h-40 rounded-full ${
          darkMode ? 'bg-yellow-600/5' : 'bg-yellow-600/3'
