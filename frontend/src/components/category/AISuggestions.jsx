    ...
    <>
      ...
      <div className="space-y-3 mb-6">
        {safeSuggestions.map((suggestion) => (
          <div
            key={suggestion.id}
            className={`${darkMode ? 'bg-gradient-to-r from-purple-900/20 to-pink-900/20 border-purple-800' : 'bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200'} border rounded-2xl overflow-hidden`}
          >
            <div className="p-4">
              ...
              <div className="flex gap-2">
                {!userData.isPremium && userData.plansUsedThisMonth >= 5 ? (
                  <button 
                    ...
                    className="flex-1 py-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-400 rounded-xl text-xs font-medium"
                  >
                    🔒 Débloquer (Premium)
                  </button>
                ) : (
                  <>
                    <button 
                      ...
                      className="flex-1 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl text-xs font-medium transition-all active:scale-95"
                    >
                      Utiliser maintenant
                    </button>
                    <button 
