...
const EmailVerificationScreen = ({
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-black' : 'bg-white'} flex flex-col justify-center px-6 py-8`}>
      ...
      <div className="text-center mb-8">
        {canResend ? (
          <button
            ...
            className={`text-sm ${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-500'} transition-colors underline`}
          >
            📮 Renvoyer le code
          </button>
        ) : (
      ...
      <div className={`mt-6 p-4 rounded-xl ${darkMode ? 'bg-blue-900/20' : 'bg-blue-50'} border border-blue-500/20`}>
        <h4 className={`font-semibold ${darkMode ? 'text-blue-300' : 'text-blue-700'} mb-2 text-center`}>
          Pourquoi vérifier ton email ?
        </h4>
        <div className="space-y-1 text-xs">
          <div className={`flex items-center space-x-2 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>
            <span>🔔</span>
