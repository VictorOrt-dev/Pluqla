...
const { createFinancialRateLimit, validateFinancialInput, gdprCompliance, monitorSuspiciousActivity } = require('../middleware/securityMiddleware');
...
router.post('/authorize',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 60000, max: 5 }), // 5 attempts per minute
  [
    body('provider')
...
router.post('/callback',
  createFinancialRateLimit({ windowMs: 60000, max: 10 }), // 10 callbacks per minute
  [
    body('code')
...
router.delete('/disconnect/:provider',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 60000, max: 3 }), // 3 disconnections per minute
  [
    param('provider')
      .isIn(['bridge', 'budgetinsight', 'tink'])
      .withMessage('Invalid provider')
...
router.get('/status',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 60000, max: 20 }), // 20 status checks per minute
...
router.post('/refresh/:provider',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 60000, max: 5 }), // 5 refresh attempts per minute
  [
    param('provider')
      .isIn(['bridge', 'budgetinsight', 'tink'])
      .withMessage('Invalid provider')
...
router.post('/consent',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 300000, max: 3 }), // 3 consent updates per 5 minutes
  [
    body('consentType')
...
router.get('/consent',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 60000, max: 10 }), // 10 consent checks per minute
  async (req, res) => {
    try {
      const userId = req.user.id;
      const gdprService = require('../services/gdprService');
...
router.get('/providers',
  authenticateToken,
  createFinancialRateLimit({ windowMs: 300000, max: 10 }), // 10 requests per 5 minutes
  (req, res) => {
    try {
      const providers = [
        {
