...
class AIService {
  ...
  async generateAISuggestions(category, context, language = 'fr') {
    ...
    try {
      if (this.openai) {
        const response = await this.openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'Tu es un assistant IA spécialisé dans les conseils d\'économies personnalisées pour l\'application +Clair. Réponds UNIQUEMENT en JSON valide avec un tableau de suggestions. Ne pas inclure de texte supplémentaire.'
            },
            {
              role: 'user',
              content: prompt
      ...
      logger.error('Erreur API IA externe:', error);
      throw error;
    }
  }
