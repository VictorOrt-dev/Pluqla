/**
 * Auto-Generated OpenAPI Documentation Generator
 *
 * Scans route files and generates comprehensive OpenAPI 3.0 specification
 * Includes authentication flows, example requests/responses, and endpoint descriptions
 */

const fs = require('fs');
const path = require('path');

class OpenAPIGenerator {
  constructor() {
    this.spec = {
      openapi: '3.0.0',
      info: {
        title: 'Pluqla API',
        version: '1.0.0',
        description: 'API pour l\'application Pluqla - Économies intelligentes avec IA',
        contact: {
          name: 'Équipe Pluqla',
          email: 'support@pluqla.com'
        },
        license: {
          name: 'Proprietary',
          url: 'https://pluqla.com/license'
        }
      },
      servers: [
        {
          url: 'http://localhost:3004/api',
          description: 'Serveur de développement'
        },
        {
          url: 'https://api.pluqla.com/api',
          description: 'Serveur de production'
        }
      ],
      paths: {},
      components: {
        schemas: this.getSchemas(),
        securitySchemes: {
          BearerAuth: {
            type: 'http',
            scheme: 'bearer',
            bearerFormat: 'JWT',
            description: 'JWT token obtained from /auth/login endpoint'
          }
        },
        responses: this.getCommonResponses(),
        parameters: this.getCommonParameters()
      },
      security: [
        {
          BearerAuth: []
        }
      ],
      tags: this.getTags()
    };

    this.routePatterns = this.getRoutePatterns();
  }

  /**
   * Get common schemas used across the API
   */
  getSchemas() {
    return {
      ApiResponse: {
        type: 'object',
        properties: {
          success: {
            type: 'boolean',
            description: 'Indicates if the request was successful'
          },
          message: {
            type: 'string',
            description: 'Human-readable message'
          },
          data: {
            type: 'object',
            description: 'Response data'
          },
          meta: {
            type: 'object',
            properties: {
              timestamp: { type: 'string', format: 'date-time' },
              apiVersion: { type: 'string' },
              status: { type: 'integer' }
            }
          },
          errors: {
            type: 'array',
            items: { type: 'string' }
          }
        },
        required: ['success', 'message']
      },
      User: {
        type: 'object',
        properties: {
          id: { type: 'string', format: 'uuid' },
          email: { type: 'string', format: 'email' },
          name: { type: 'string' },
          profilePicture: { type: 'string', nullable: true },
          level: { type: 'integer', minimum: 1 },
          xp: { type: 'integer', minimum: 0 },
          streak: { type: 'integer', minimum: 0 },
          createdAt: { type: 'string', format: 'date-time' },
          updatedAt: { type: 'string', format: 'date-time' }
        },
        required: ['id', 'email', 'name']
      },
      Transaction: {
        type: 'object',
        properties: {
          id: { type: 'string', format: 'uuid' },
          userId: { type: 'string', format: 'uuid' },
          amount: { type: 'number', format: 'decimal' },
          description: { type: 'string' },
          category: { type: 'string' },
          type: { type: 'string', enum: ['income', 'expense', 'saving'] },
          date: { type: 'string', format: 'date-time' },
          createdAt: { type: 'string', format: 'date-time' }
        },
        required: ['amount', 'description', 'category', 'type']
      },
      FinancialSummary: {
        type: 'object',
        properties: {
          totalIncome: { type: 'number', format: 'decimal' },
          totalExpenses: { type: 'number', format: 'decimal' },
          totalSavings: { type: 'number', format: 'decimal' },
          netWorth: { type: 'number', format: 'decimal' },
          monthlyGoal: { type: 'number', format: 'decimal' },
          goalProgress: { type: 'number', minimum: 0, maximum: 100 },
          period: { type: 'string', enum: ['daily', 'weekly', 'monthly', 'yearly'] }
        }
      },
      AISuggestion: {
        type: 'object',
        properties: {
          id: { type: 'string' },
          title: { type: 'string' },
          description: { type: 'string' },
          category: { type: 'string' },
          potentialSavings: { type: 'number', format: 'decimal' },
          difficulty: { type: 'string', enum: ['easy', 'medium', 'hard'] },
          confidence: { type: 'number', minimum: 0, maximum: 1 }
        }
      },
      Strike: {
        type: 'object',
        properties: {
          id: { type: 'string', format: 'uuid' },
          userId: { type: 'string', format: 'uuid' },
          currentStreak: { type: 'integer', minimum: 0 },
          longestStreak: { type: 'integer', minimum: 0 },
          lastActivityDate: { type: 'string', format: 'date' },
          streakType: { type: 'string', enum: ['daily', 'weekly'] }
        }
      },
      Error: {
        type: 'object',
        properties: {
          success: { type: 'boolean', example: false },
          message: { type: 'string' },
          error: { type: 'string' },
          status: { type: 'integer' }
        }
      }
    };
  }

  /**
   * Get common response templates
   */
  getCommonResponses() {
    return {
      200: {
        description: 'Successful response',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/ApiResponse' }
          }
        }
      },
      400: {
        description: 'Bad Request',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Données invalides',
              error: 'Validation failed',
              status: 400
            }
          }
        }
      },
      401: {
        description: 'Unauthorized',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Token d\'authentification requis',
              error: 'Unauthorized',
              status: 401
            }
          }
        }
      },
      403: {
        description: 'Forbidden',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Accès interdit',
              error: 'Forbidden',
              status: 403
            }
          }
        }
      },
      404: {
        description: 'Not Found',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Ressource introuvable',
              error: 'Not Found',
              status: 404
            }
          }
        }
      },
      429: {
        description: 'Rate Limit Exceeded',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Trop de requêtes',
              error: 'Rate limit exceeded',
              status: 429
            }
          }
        }
      },
      500: {
        description: 'Internal Server Error',
        content: {
          'application/json': {
            schema: { $ref: '#/components/schemas/Error' },
            example: {
              success: false,
              message: 'Erreur interne du serveur',
              error: 'Internal server error',
              status: 500
            }
          }
        }
      }
    };
  }

  /**
   * Get common parameters
   */
  getCommonParameters() {
    return {
      PageParam: {
        name: 'page',
        in: 'query',
        description: 'Page number for pagination',
        schema: { type: 'integer', minimum: 1, default: 1 }
      },
      LimitParam: {
        name: 'limit',
        in: 'query',
        description: 'Number of items per page',
        schema: { type: 'integer', minimum: 1, maximum: 100, default: 10 }
      },
      DateFromParam: {
        name: 'from',
        in: 'query',
        description: 'Start date for filtering (ISO 8601)',
        schema: { type: 'string', format: 'date' }
      },
      DateToParam: {
        name: 'to',
        in: 'query',
        description: 'End date for filtering (ISO 8601)',
        schema: { type: 'string', format: 'date' }
      }
    };
  }

  /**
   * Get API tags for grouping endpoints
   */
  getTags() {
    return [
      {
        name: 'Authentication',
        description: 'Endpoints for user authentication and authorization'
      },
      {
        name: 'Users',
        description: 'User profile and account management'
      },
      {
        name: 'Transactions',
        description: 'Financial transaction management'
      },
      {
        name: 'Financial',
        description: 'Financial data and analytics'
      },
      {
        name: 'AI',
        description: 'AI-powered suggestions and recommendations'
      },
      {
        name: 'Analytics',
        description: 'Analytics and reporting endpoints'
      },
      {
        name: 'Categories',
        description: 'Transaction categories management'
      },
      {
        name: 'Strikes',
        description: 'User streak and gamification system'
      },
      {
        name: 'Performance',
        description: 'Performance monitoring and metrics'
      },
      {
        name: 'Uploads',
        description: 'File upload endpoints'
      }
    ];
  }

  /**
   * Define route patterns and their specifications
   */
  getRoutePatterns() {
    return {
      // Authentication endpoints
      'POST /auth/register': {
        tags: ['Authentication'],
        summary: 'Register new user',
        description: 'Create a new user account with email and password',
        security: [],
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  email: { type: 'string', format: 'email' },
                  password: { type: 'string', minLength: 8 },
                  name: { type: 'string', minLength: 2 }
                },
                required: ['email', 'password', 'name']
              }
            }
          }
        },
        responses: {
          201: {
            description: 'User created successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            user: { $ref: '#/components/schemas/User' },
                            accessToken: { type: 'string' },
                            refreshToken: { type: 'string' }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          400: { $ref: '#/components/responses/400' },
          409: {
            description: 'Email already exists',
            content: {
              'application/json': {
                schema: { $ref: '#/components/schemas/Error' },
                example: {
                  success: false,
                  message: 'Un compte existe déjà avec cet email',
                  status: 409
                }
              }
            }
          }
        }
      },

      'POST /auth/login': {
        tags: ['Authentication'],
        summary: 'User login',
        description: 'Authenticate user with email and password',
        security: [],
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  email: { type: 'string', format: 'email' },
                  password: { type: 'string' }
                },
                required: ['email', 'password']
              }
            }
          }
        },
        responses: {
          200: {
            description: 'Login successful',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            user: { $ref: '#/components/schemas/User' },
                            accessToken: { type: 'string' },
                            refreshToken: { type: 'string' }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          400: { $ref: '#/components/responses/400' },
          401: {
            description: 'Invalid credentials',
            content: {
              'application/json': {
                schema: { $ref: '#/components/schemas/Error' },
                example: {
                  success: false,
                  message: 'Email ou mot de passe incorrect',
                  status: 401
                }
              }
            }
          }
        }
      },

      'POST /auth/refresh': {
        tags: ['Authentication'],
        summary: 'Refresh access token',
        description: 'Get a new access token using refresh token',
        security: [],
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  refreshToken: { type: 'string' }
                },
                required: ['refreshToken']
              }
            }
          }
        },
        responses: {
          200: {
            description: 'Token refreshed successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            accessToken: { type: 'string' },
                            refreshToken: { type: 'string' }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // User endpoints
      'GET /users/me': {
        tags: ['Users'],
        summary: 'Get current user profile',
        description: 'Retrieve the authenticated user\'s profile information',
        responses: {
          200: {
            description: 'User profile retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: { $ref: '#/components/schemas/User' }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      'PUT /users/profile': {
        tags: ['Users'],
        summary: 'Update user profile',
        description: 'Update the authenticated user\'s profile information',
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  name: { type: 'string', minLength: 2 },
                  email: { type: 'string', format: 'email' }
                }
              }
            }
          }
        },
        responses: {
          200: {
            description: 'Profile updated successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: { $ref: '#/components/schemas/User' }
                      }
                    }
                  ]
                }
              }
            }
          },
          400: { $ref: '#/components/responses/400' },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // Financial endpoints
      'GET /financial/summary': {
        tags: ['Financial'],
        summary: 'Get financial summary',
        description: 'Retrieve user\'s financial overview and statistics',
        parameters: [
          {
            name: 'period',
            in: 'query',
            description: 'Time period for the summary',
            schema: { type: 'string', enum: ['daily', 'weekly', 'monthly', 'yearly'], default: 'monthly' }
          }
        ],
        responses: {
          200: {
            description: 'Financial summary retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: { $ref: '#/components/schemas/FinancialSummary' }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // Transaction endpoints
      'GET /transactions': {
        tags: ['Transactions'],
        summary: 'Get user transactions',
        description: 'Retrieve paginated list of user transactions',
        parameters: [
          { $ref: '#/components/parameters/PageParam' },
          { $ref: '#/components/parameters/LimitParam' },
          { $ref: '#/components/parameters/DateFromParam' },
          { $ref: '#/components/parameters/DateToParam' },
          {
            name: 'category',
            in: 'query',
            description: 'Filter by transaction category',
            schema: { type: 'string' }
          },
          {
            name: 'type',
            in: 'query',
            description: 'Filter by transaction type',
            schema: { type: 'string', enum: ['income', 'expense', 'saving'] }
          }
        ],
        responses: {
          200: {
            description: 'Transactions retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            transactions: {
                              type: 'array',
                              items: { $ref: '#/components/schemas/Transaction' }
                            },
                            pagination: {
                              type: 'object',
                              properties: {
                                page: { type: 'integer' },
                                limit: { type: 'integer' },
                                total: { type: 'integer' },
                                totalPages: { type: 'integer' }
                              }
                            }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      'POST /transactions': {
        tags: ['Transactions'],
        summary: 'Create new transaction',
        description: 'Add a new financial transaction',
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  amount: { type: 'number', format: 'decimal' },
                  description: { type: 'string', minLength: 1 },
                  category: { type: 'string' },
                  type: { type: 'string', enum: ['income', 'expense', 'saving'] },
                  date: { type: 'string', format: 'date-time' }
                },
                required: ['amount', 'description', 'category', 'type']
              }
            }
          }
        },
        responses: {
          201: {
            description: 'Transaction created successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: { $ref: '#/components/schemas/Transaction' }
                      }
                    }
                  ]
                }
              }
            }
          },
          400: { $ref: '#/components/responses/400' },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // AI endpoints
      'GET /ai/suggestions': {
        tags: ['AI'],
        summary: 'Get AI suggestions',
        description: 'Retrieve AI-powered savings suggestions based on user data',
        parameters: [
          {
            name: 'category',
            in: 'query',
            description: 'Filter suggestions by category',
            schema: { type: 'string' }
          },
          {
            name: 'limit',
            in: 'query',
            description: 'Maximum number of suggestions to return',
            schema: { type: 'integer', minimum: 1, maximum: 20, default: 5 }
          }
        ],
        responses: {
          200: {
            description: 'AI suggestions retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            suggestions: {
                              type: 'array',
                              items: { $ref: '#/components/schemas/AISuggestion' }
                            }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // Strikes endpoints
      'GET /strikes/current': {
        tags: ['Strikes'],
        summary: 'Get current user streak',
        description: 'Retrieve the user\'s current streak information',
        responses: {
          200: {
            description: 'Streak information retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: { $ref: '#/components/schemas/Strike' }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      },

      // Performance endpoints
      'GET /performance/metrics': {
        tags: ['Performance'],
        summary: 'Get performance metrics',
        description: 'Retrieve API and database performance metrics (requires authentication)',
        responses: {
          200: {
            description: 'Performance metrics retrieved successfully',
            content: {
              'application/json': {
                schema: {
                  allOf: [
                    { $ref: '#/components/schemas/ApiResponse' },
                    {
                      properties: {
                        data: {
                          type: 'object',
                          properties: {
                            summary: {
                              type: 'object',
                              properties: {
                                totalApiCalls: { type: 'integer' },
                                totalDbQueries: { type: 'integer' },
                                slowApiCalls: { type: 'integer' },
                                slowDbQueries: { type: 'integer' }
                              }
                            },
                            topSlowApiEndpoints: {
                              type: 'array',
                              items: {
                                type: 'object',
                                properties: {
                                  endpoint: { type: 'string' },
                                  averageTime: { type: 'number' },
                                  totalCalls: { type: 'integer' },
                                  slowCalls: { type: 'integer' }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          401: { $ref: '#/components/responses/401' }
        }
      }
    };
  }

  /**
   * Generate complete OpenAPI specification
   */
  generate() {
    // Add all route patterns to paths
    Object.entries(this.routePatterns).forEach(([routeKey, spec]) => {
      const [method, path] = routeKey.split(' ');
      const pathKey = path.replace(/\/:/g, '/{').replace(/([^}])$/, '$1}');

      if (!this.spec.paths[pathKey]) {
        this.spec.paths[pathKey] = {};
      }

      this.spec.paths[pathKey][method.toLowerCase()] = spec;
    });

    return this.spec;
  }

  /**
   * Save OpenAPI spec to file
   */
  save(outputPath) {
    const spec = this.generate();
    const json = JSON.stringify(spec, null, 2);

    fs.writeFileSync(outputPath, json, 'utf8');
    console.log(`✅ OpenAPI specification generated: ${outputPath}`);
    console.log(`📊 Generated ${Object.keys(spec.paths).length} paths with ${this.countEndpoints()} endpoints`);

    return outputPath;
  }

  /**
   * Count total endpoints
   */
  countEndpoints() {
    const spec = this.generate();
    let count = 0;

    Object.values(spec.paths).forEach(path => {
      count += Object.keys(path).length;
    });

    return count;
  }
}

// Generate OpenAPI spec if run directly
if (require.main === module) {
  const generator = new OpenAPIGenerator();
  const outputPath = path.join(__dirname, '../openapi.json');

  try {
    generator.save(outputPath);
    console.log('\n🎉 OpenAPI documentation generated successfully!');
    console.log(`📖 View documentation at: http://localhost:3004/api-docs`);
  } catch (error) {
    console.error('❌ Failed to generate OpenAPI specification:', error);
    process.exit(1);
  }
}

module.exports = { OpenAPIGenerator };