...
const ActivityRecommendations = ({
  ...
  return (
    <div className="space-y-6">
        ...
        {isLoading && (
          <div className={`group relative p-8 backdrop-blur-xl rounded-3xl border transition-all duration-500 ${
            ...
            <div className="relative z-10 text-center space-y-4">
              <div className="relative">
                <div className="animate-spin w-12 h-12 border-3 border-red-500/30 border-t-red-500 rounded-full mx-auto"></div>
            ...
            <div
              ...
              className={`group relative p-6 backdrop-blur-xl rounded-3xl border transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl ${
                darkMode
                  ? 'bg-gray-900/60 border-gray-700/50 shadow-lg'
                  : 'bg-white/70 border-gray-200/60 shadow-md'
              } ring-2 ring-red-500/25 shadow-red-500/10`}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-gradient-to-r from-red-500 to-indigo-600 rounded-2xl shadow-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 mb-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          <span className="text-xs bg-gradient-to-r from-red-500 to-indigo-600 text-white px-3 py-1 rounded-full font-semibold">
                            🤖 IA Personnalisée
                          </span>
                        </div>
                      </div>
                    ...
                    <div className={`p-4 backdrop-blur-sm rounded-2xl border ${
                      darkMode
                        ? 'bg-gradient-to-r from-red-900/30 to-indigo-900/30 border-red-700/50'
                        : 'bg-gradient-to-r from-red-50/80 to-indigo-50/80 border-orange-200/60'
                    } shadow-lg`}>
                      <div className="flex items-start space-x-3">
                        <div className="p-1.5 bg-gradient-to-r from-red-500 to-indigo-600 rounded-lg">
                          <span className="text-white text-sm">✨</span>
                        </div>
                        <div className="flex-1">
                          <p className={`text-sm font-semibold ${darkMode ? 'text-orange-300' : 'text-red-800'}`}>
                            Suggestion adaptée à votre profil
                          </p>
                          <p className={`text-xs mt-1 ${darkMode ? 'text-orange-400' : 'text-red-800'}`}>
      ...
      <div className={`group relative p-6 backdrop-blur-xl rounded-3xl border transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl ${
        darkMode
          ? 'bg-gradient-to-r from-red-900/40 to-pink-900/40 border-red-700/50 shadow-lg'
          : 'bg-gradient-to-r from-red-50/80 to-pink-50/80 border-orange-200/60 shadow-md'
      }`}>
        ...
        <div className="relative z-10">
          <div className="flex items-center space-x-3 mb-6">
            <div className="p-2.5 bg-gradient-to-r from-red-500 to-orange-500 rounded-xl">
              <span className="text-white text-lg">{title.includes('Sport') ? '🏆' : '🎪'}</span>
            </div>
            <div>
              <h3 className={`text-xl font-bold bg-gradient-to-r ${
                darkMode
                  ? 'from-orange-200 via-pink-200 to-orange-300'
                  : 'from-red-900 via-orange-800 to-red-700'
              } bg-clip-text text-transparent`}>
                {title.includes('Sport') ? 'Objectifs sportifs' : 'Idées de sorties'}
              </h3>
              <p className={`text-sm ${darkMode ? 'text-orange-300' : 'text-red-700'}`}>
            ...
            {aiSuggestions.slice(0, 2).map((suggestion, idx) => (
              <div
                ...
                className={`flex justify-between items-center p-3 backdrop-blur-sm rounded-2xl border transition-all duration-300 hover:scale-[1.02] ${
                  darkMode
                    ? 'bg-red-800/20 border-red-600/30 hover:bg-red-700/30'
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <span className={`text-sm font-medium ${darkMode ? 'text-orange-200' : 'text-red-700'}`}>
                    {suggestion.title || suggestion.name || 'Suggestion IA'}
                  </span>
                </div>
                <div className="flex items-center space-x-2">
                  <span className={`text-sm font-bold ${darkMode ? 'text-orange-300' : 'text-red-800'}`}>
          ...
          <div className={`p-4 backdrop-blur-sm rounded-2xl border-t ${
            darkMode
              ? 'border-red-600/30 bg-red-800/10'
              : 'border-orange-300/50 bg-white/40'
          }`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <span className="text-lg">🎯</span>
                <span className={`text-sm font-medium ${darkMode ? 'text-orange-300' : 'text-red-700'}`}>
