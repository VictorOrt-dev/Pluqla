  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 🔥 FORCER LA VÉRIFICATION D'AUTHENTIFICATION IMMÉDIATEMENT
        try {
          await checkAuthState();
          console.log('✅ État d\'authentification mis à jour après login/signup');
        } catch (error) {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-black' : 'bg-white'} flex flex-col justify-center px-6 py-8`}>
      ...
      <div className="text-center mb-8">
        <div className="w-16 h-16 bg-gradient-to-br pluqla-gradient-main rounded-2xl flex items-center justify-center mx-auto mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-6">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className={`absolute right-3 top-1/2 transform -translate-y-1/2 ${darkMode ? 'text-gray-400' : 'text-gray-600'} hover:text-red-500`}
            >
              {showPassword ? '🙈' : '👁️'}
            </button>
          </div>
        ...
        <button
          ...
          className={`w-full py-3 pluqla-btn-primary text-white font-semibold rounded-xl transition-all ${
            isSubmitting
      ...
      <div className="text-center mb-6">
        <button
          onClick={switchMode}
          className={`text-sm ${darkMode ? 'text-red-400 hover:text-blue-300' : 'text-red-600 hover:text-red-500'} transition-colors`}
        >
