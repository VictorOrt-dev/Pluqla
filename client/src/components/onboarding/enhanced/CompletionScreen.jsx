...
const CompletionScreen = ({
  ...
  const completeOnboarding = () => {
    ...
    // 🎯 Utiliser onNext pour déclencher la logique du hook useOnboarding
    // Ceci va appeler completeOnboarding dans le hook qui gère la redirection correctly
    onNext();
  };
  ...
  const getLevelInfo = (level) => {
    const levels = {
      ...
      2: { title: 'Économiseur Confirmé', color: 'pluqla-gradient-main', icon: '⭐' },
      3: { title: 'Expert Économies', color: 'from-red-500 to-orange-500', icon: '💎' }
    };
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-black' : 'bg-white'} flex flex-col justify-center items-center px-6 py-8 relative overflow-hidden`}>
      ...
      <div className="relative z-20 w-full max-w-sm">
        ...
        <button
          ...
          className={`w-full py-4 pluqla-btn-primary text-white font-semibold rounded-2xl shadow-xl transition-all hover:scale-[1.02] active:scale-95 ${
        ...
        {[...Array(15)].map((_, i) => (
          <div
            ...
            className={`absolute text-4xl animate-pulse ${
              i % 3 === 0 ? 'text-red-500' : i % 3 === 1 ? 'text-red-700' : 'text-green-500'
            }`}
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
