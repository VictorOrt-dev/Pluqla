...
export const AuthProvider = ({ children }) => {
  ...
  const apiCall = useCallback(async (endpoint, options = {}) => {
    const { accessToken } = (state.tokens && state.tokens.accessToken) ? state.tokens : loadTokensFromStorage();
  ...
  // Initialize API adapter and financial API after all functions are defined
  useEffect(() => {
    ...
    apiAdapter.init(authContextValue);
    initFinancialApi(authContextValue);

    secureLogger.info('Auth context initialized with API services');
  }, [state.authState, state.user, state.tokens, state.isRefreshing, refreshAuthToken, logout]);
